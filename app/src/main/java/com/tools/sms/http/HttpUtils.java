...
import com.tools.sms.bean.xmlEntity;
...
//	    //发送调用
//		xml=t.sendMessage("1111", "qqqq", "mima", "15201131591", "测试剩余与使用1", "").toString();
...
//		System.out.println("错误代码："+xmlentity.getError());
    ...
    //查询余额
    private StringBuffer SelSum(String userid, String account, String password) {
        try {
    ...
    public StringBuffer sendMessage(String userid, String account, String password, String mobile, String content, String sendTime) {
        try {
            ...
            url = new URL("http://112.74.36.247/sms.aspx?action=send&userid="
                    + userid + "&account=" + account + "&password=" + password + "&mobile="
                    + mobile + "&content=" + send_content + "&sendTime=" + sendTime + "");
            con = (HttpURLConnection) url.openConnection();
            br = new BufferedReader(new InputStreamReader(con.getInputStream(), "UTF-8"));
            while ((line = br.readLine()) != null) {
                //追加字符串获得XML形式的字符串
